...
- Open the "FOXI3_plink_finalcoded.vcf" as a text edit file
...
setwd("~/Downloads")
dat_hap_FOXI3 <- read.csv("FOXI3_plink_finalcoded.csv", header=TRUE)

# organize the dataset 
data_long_FOXI3 <- gather(dat_hap_FOXI3, Sample, Allele, Affenpinscher01:YorkshireTerrier77, factor_key=TRUE)

#Make my variables in factor format
data_long_FOXI3$POS <- as.factor(data_long_FOXI3$POS)
data_long_FOXI3$Allele <- as.factor(data_long_FOXI3$Allele)

#Plot with geom_tile entire FOXI3 gene
p1 <- ggplot(data_long_FOXI3,aes(x=POS,y=Sample,fill=Allele))+
      geom_tile() 
...
ggsave("FOXI3_gene.tiff", width=10,height=3)
